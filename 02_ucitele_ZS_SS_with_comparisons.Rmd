---
title: "Výsledky z dotazníku pro učitele"
team: výzkumný tým SCHOLA EMPIRICA
authors:
  - "Magdaléna Klimešová^[Korespondenční autor. Kontakt: klimesova@scholaempirica.org]"
  - Jaromír Mazák
  - Jan Netík
  - Aleš Vomáčka
  - Martina Koutná
  - Marek Havrda
date: '`r reschola::as_czech_date(Sys.time())`' # remove leading zero
params:
  redizo: "600046389" # there must be a value, to smoothly compile outside Build script
  fig_asp_body: .6
  fig_asp_appdx: .4
  alpha_lower: .6
output: 
  reschola::schola_pdf:
    keep_tex: false # for debugging
    keep_md: false # for debugging
    template: templates/schola_template.tex
    num_format: cs
lang: cs # cs or en
toc-title: "Obsah zprávy"
logo: logos/scholaLogoNew # for path with _ use "`logos/schola_logo.pdf`{=latex}"
logo_height: 12pt
logo_client: logos/eduzmena
logo_client_height: 25pt
head_height: false # may be used if fancyhdr complains 
---

```{r setup}
library(knitr)
opts_knit$set(eval.after = "fig.cap") # evaluate fig.caps after the relevant object exist
knitr::opts_chunk$set(fig.asp = params$fig_asp_body, echo = FALSE, cache = FALSE)
options(scipen = 10) # force R to show full numbers, not scientific notation

library(tidyverse)
library(scales)
library(magrittr)
library(here)
library(reschola)
library(lubridate)
set_reschola_ggplot_fonts()
theme_set(theme_schola())
source("shared.R", local = TRUE) # use the environment of the source (because of Makefile)
```

```{r data}
# complete data
d <- read_rds(here("data-processed", "ucitele_ZS_SS_wave1.rds"))

# add selection variable indicating selected school
d %<>% mutate(digest = if_else(red_izo == params$redizo, TRUE, FALSE))

# test if there sufficient data for selected school
if (d %>% filter(digest) %>% nrow() %>% magrittr::is_less_than(4)) {
  stop("For RED IZO ", params$redizo, ", there is less than 4 observations. ",
    "Possible anonymity breach. Falling back...",
    call. = FALSE
  )
}

# get human readable REDIZO & school name from the big database for further use
instit_digest <- read_rds(here("data-input/institutions_ms_zs_ss.rds"))

selected_school_name <- instit_digest %>%
  filter(red_izo == params$redizo) %>%
  distinct(across(c(red_izo, fullname, name))) %>%
  pull(fullname) # some shortened name are useles (e.g. "základní škola"...)
```



```{r missingsAndEscaped}
d %<>% naniar::add_n_miss(matches("^s\\d.*")) # variables beginning with s followed by int

d %<>% rowwise %>%
  mutate(n_escape = sum(str_detect(c_across(where(is.factor)), "[N\n]evím"), na.rm = TRUE)) %>%
  ungroup()

# delete those stats in not submitted cases
d[is.na(d$submitted), c("n_escape", "n_miss_vars")] <- NA
```


```{r respStatsAndfiltering}
# resp_stats
resp_stats <- d %>%
  mutate(
    n_escape_outlier = scale(n_escape) > qnorm(1 - (.05 / 2)),
    n_miss_outlier = scale(n_miss_vars) > qnorm(1 - (.05 / 2))
  ) %>%
  group_by(digest, red_izo) %>%
  summarize(
    n_total = n(),
    n_invited = number_invited %>% unique(),
    n_opened = sum(!is.na(cur_data()$opened)),
    n_submitted = sum(!is.na(cur_data()$submitted)),
    n_opened_not_sumbitted = sum(!is.na(opened) & is.na(submitted)),
    n_escapes_outliers = sum(n_escape_outlier),
    n_miss_outliers = sum(n_miss_vars)
  ) %>%
  group_by(digest) %>%
  summarise(across(-red_izo,
    list(
      med = ~ median(.x, na.rm = TRUE),
      sum = ~ sum(.x, na.rm = TRUE)
    ),
    .names = "{.col}_{.fn}"
  ))

n_miss_outl <- resp_stats %>%
  filter(digest) %>%
  pull(n_miss_outliers_sum)
n_esc_outl <- resp_stats %>%
  filter(digest) %>%
  pull(n_escapes_outliers_sum)

any_outliers <- sum(n_miss_outl, n_esc_outl) != 0



# filter current school for respondents stats
curr_school <- d %>% filter(digest)

total_teachers <- nrow(curr_school) # can be higher than invited

total_invited <- curr_school$number_invited %>% unique()

opened_count <- curr_school %>%
  filter(!is.na(opened)) %>%
  nrow()

opened_not_sumbitted <- curr_school %>%
  filter(!is.na(opened) & is.na(submitted)) %>%
  nrow()

completed <- opened_count - opened_not_sumbitted

response_rate_invited <- percent(completed / total_invited, 1, suffix = "\u00A0%", decimal.mark = ",")
response_rate_effective <- percent(completed / total_teachers, 1, suffix = "\u00A0%", decimal.mark = ",")

last_pages_not_submitted <- curr_school %>%
  filter(!is.na(opened) & is.na(submitted)) %>%
  pull(last_page)

open_to_close_dur <- curr_school %>% # in minutes
  filter(!is.na(opened) & is.na(submitted)) %>%
  mutate(dur_to_close = closed - opened) %>%
  pull(dur_to_close) %>%
  as.numeric() %>%
  divide_by(60)

open_to_submit_dur <- curr_school %>% # in minutes
  filter(!is.na(opened) & !is.na(submitted)) %>%
  mutate(dur_to_close = closed - opened) %>%
  pull(dur_to_close) %>%
  as.numeric()

# filter only submitted
d %<>% filter(!is.na(submitted))

# rowwise variables

# create number of missing & number of
d %<>% naniar::add_n_miss(starts_with("s")) # already present, override

d %<>% rowwise %>%
  mutate(n_escape = sum(str_detect(c_across(where(is.factor)), "[N\n]evím"), na.rm = TRUE)) %>%
  ungroup()

# colwise measures, escape options outliers among the sample
escape_med <- median(d$n_escape, na.rm = TRUE)
escape_idx <- as.logical(scale(d$n_escape) > 1.96)
n_escapes_outliers <- d[escape_idx, ]$n_escape # number of escaped, not indices
```

```{r basicInfo}

opened_minmax <- d %>% summarize(
  min = min(opened, na.rm = TRUE),
  max = max(opened, na.rm = TRUE)
)

survey_time_int <- czech_date_interval(opened_minmax$min, opened_minmax$max)

opened_minmax_curr_school <- d %>%
  filter(digest) %>%
  summarize(
    min = min(opened, na.rm = TRUE),
    max = max(opened, na.rm = TRUE)
  )

survey_time_int_curr_school <- czech_date_interval(opened_minmax_curr_school$min, opened_minmax_curr_school$max)
```

<!-- subtitle comes after we know the name of the institution selected -->

---
subtitle: |
  `r selected_school_name` \linebreak RED IZO: `r params$redizo`
abstract: |
  Zpráva seznamuje s výsledky šetření mezi učiteli v rámci projektu Eduzměna, u kterých jsme mapovali situaci v celé řadě oblastí.
  
  Šlo zejména o vnímání školního prostředí, sebehodnocení pedagogických dovedností a víru v dopady své práce, otázky seberozvoje, mentoringu a účinku zpětné vazby, vnímané autonomie a podpory ze strany vedení školy, komunikaci a spolupráci s různými aktéry, spokojenost s prací a potenciální pracovní stresory, vnímání odměny, prestiže pedagogické profese a školy jako celku.
  
  Zmíněné oblasti jsme mapovali pomocí online dotazníku, jehož sběr na Vaší škole probíhal `r survey_time_int_curr_school`, celé dotazníkové šetření se všemi zúčastněnými školami pak v rozmezí `r survey_time_int`.
  
  V první části naleznete krátký komentář k interpretaci použitých grafů, poté se věnujeme rovnou tomu nejzajímavějšímu a sice výsledkům, které jsou vždy členěny na Vaší a ostatní školy, abyste si mohli stav u Vás snadno porovnat s průměrem ostatních zapojených škol. Informace o respondentech, včetně počtu zapojených vyučujících a škol, naleznete za samotnými výsledky. Zpráva obsahuje také přílohu s detailním pohledem na otázky, které v těle zprávy prezentujeme souhrnně v tzv. škálách (viz dále).
  
  Přejeme Vám zajímavé a inspirativní čtení a věříme, že zpráva poslouží k dalšímu rozvoji Vaší školy a školního prostředí.
---


```{r labs}
# this chunk should be used for cetral item wording edits (shortening etc.)
item_labs_list <- read_rds(here("data-processed/ucitele_ZS_SS_wave1_labels.rds"))

# the most important is the part in brackets, so extract it
item_labs_list %<>% map(~ .x %>%
  if_else(str_detect(., "[\\[\\]]"), str_extract(., "(?<=\\[).*(?=\\])"), .) %>%
  if_else(str_detect(., "\\["), str_extract(., ".*(?=\\[)"), .) %>%
  str_trim())
```

```{r labsEditing}
# when you change the CSV created there, just rerun this chunk to take effect

# export labels as csv for easier editing (auto created if does not exist)
if (!file.exists(here("data-processed", "ucitele_ZS_SS_wave1_labels.csv"))) {
  item_labs_list %>%
    unlist() %>%
    enframe(name = "code", value = "wording") %>%
    write_excel_csv2(here("data-processed", "ucitele_ZS_SS_wave1_labels.csv"))
}

# import edited labels
item_labs_edited <- read_delim(here("data-processed/ucitele_ZS_SS_wave1_labels.csv"),
  delim = ";", col_types = cols(.default = col_character())
)

# write edited labels as the main ones
item_labs_list <- item_labs_edited$wording %>%
  str_replace_all("(?<!\\w{2})\\s", "\u00A0") %>% # nbsp at prepositions
  as.list() %>%
  set_names(item_labs_edited$code)
```

```{r factorLevels}
# make NA responses as first level wherever possible (otherwise get mad with colors)
d %<>% mutate(across(where(~ .x %>%
  levels() %>%
  str_detect("[N|n]evím") %>%
  any()), ~ fct_relevel(.x, str_subset(.x %>% levels(), "[N|n]evím"))))
```

```{r labs_view, eval=FALSE}
# nice overview in Rstudio
item_labs_list %>%
  map_chr(~.x) %>%
  as_tibble(rownames = "code") %>%
  filter(code %in% names(d)) %>%
  mutate(levels = map(code, ~ select(d, all_of(.x)) %>%
    pull() %>%
    levels())) %>%
  # unnest(levels) %>%
  view("items")
```

```{r itemCentral, include=FALSE, echo=TRUE}
# what types of response options do we have?
d %>%
  select(matches("^s\\d")) %>% # only vars beginning with "s" followed by a number
  map_chr(~ .x %>%
    levels() %>%
    str_flatten("  |  ")) %>%
  unique()

# which items are which? just FYI
d %>%
  select(matches("^s\\d")) %>% # only vars beginning with "s" followed by a number
  map_chr(~ .x %>%
    levels() %>%
    str_flatten("  |  ")) %>%
  as_tibble(rownames = "item") %>%
  nest_by(value)

# make table w/ item, its levels, number of levels, escape item presence,
# likert scale min & max, template type, fig.asp for report body & fig.asp for the appendix
# feel free to add additional useful variables,
# the values are avalable inside as list-column named "values"
items <- d %>%
  select(matches("^s\\d")) %>% # only vars beginning with "s" followed by a number
  map(~.x) %>%
  keep(~ nlevels(.x) != 0) %>% # discard empty (usualy non-factors)
  enframe("item", "values") %>%
  mutate(
    levels = map_chr(values, ~ .x %>%
      levels() %>%
      str_flatten("  |  ")),
    n_levels = map_int(values, ~ .x %>% nlevels()),
    has_dontknow = levels %>% str_detect("Nevím"),
    likert_min = 1,
    likert_max = n_levels - has_dontknow,
    resp_type = case_when( # TODO: case_when evaluates "lazily", in order, so it's possible to simplify this
      levels %>% str_detect("Žena") ~ "sex",
      levels %>% str_detect("^(?=.*Ano)(?!.*Nevybráno).*") ~ "yes_no_dontknow",
      levels %>% str_detect("^(?=.*Ano)(?!.*Nevím).*") ~ "yes_unselected",
      levels %>% str_detect("úvazek") ~ "job",
      levels %>% str_detect("(?i)souhlasím") ~ "agree",
      levels %>% str_detect("(?i)Jednou") ~ "time",
      levels %>% str_detect("značné") ~ "considerable_extent",
      levels %>% str_detect("napůl") ~ "midpoint",
      levels %>% str_detect("velké") ~ "large_extent",
      TRUE ~ "default"
    ),
    fig_asp_body = params$fig_asp_body,
    fig_asp_appdx = params$fig_asp_appdx
  )


# time related items needs to have bigger fig.asp, as the legend takes considerable amount of space
items %<>% mutate(fig_asp_appdx = if_else(resp_type == "time", fig_asp_appdx + .07, fig_asp_appdx))
```

<!-- NOTE: complete redesign on Marek's request -->
<!-- throwing out all automation, only one plot per item type w/ all items -->

\newpage

# Jak se zprávou pracovat

Jednotlivé položky dotazníku, z nichž velká část vychází z ověřených mezinárodních šetření typu TALIS či PISA, jsou sdružovány do tzv. škál (dimenzí) postihujících určité koncepty nebo oblasti. Položky se ptají na postoje respondentů, na to, jak danou otázku vnímají. Škály, které uvidíte na následujících stranách, ukazují vždy průměr z příslušných položek^[Jednotlivým kategoriím odpovědí se přiřazují přirozená, po sobě jsoucí čísla. Např. "rozhodně nesouhlasím -- rozhodně souhlasím" se očíslují 1--4, a číslo tak vyjadřuje míru souhlasu.] (konkrétní znění je k vidění v příloze). Škály prezentujeme tak, že žádoucí je vždy vyšší bodový zisk (a to i u škály "Pracovní vytížení a stres", jejíž původní bodování je pro jednotnost v tomto dotazníku "otočeno"). Podle *mediánu*^[Pokud bychom seřadili výsledky jednotlivých učitelů podle počtu bodů v dané škále, pak medián označuje hodnotu *právě uprostřed* takové řady. Pokud je učitelů sudý počet, jde o průměr prostřední dvojice učitelů.] získaných bodů také jednotlivé škály řadíme. Vizualizace všech položek dotazníku, které tvoří jednotlivé škály, jsou prezentovány v příloze.

Zpráva je sestavena s cílem poskytnout rychlý vhled. Tematické sekce, podle kterých byl dotazník členěn, jsou zde částečně potlačeny a snažíme se maximum informací prezentovat pokud možno v kompaktní grafické podobě. Ve zprávě využíváme dva typy grafů:

### Krabicový graf

Krabicový graf (pro ukázku a popis viz graf č. \@ref(fig:boxplotDesc)) přehledně zobrazuje typické hodnoty:

* silnější linka vyznačuje medián,
* šedá, vyplněná^[V zobrazení pro ostatní školy je tato část bílá.] část ohraničuje 50\ % prostředních hodnot kolem mediánu (tj. mezi 25. a 75.\ percentilem, na obrázku uvedeno jako "IQR"),
* horizontální úsečky pak poukazují na 1,5násobek tohoto rozpětí.
* Hodnoty mimo zmíněné části (vyobrazeny jako malé puntíky^[V případě, že stejnou odlehlou hodnotu zaujímá více učitelů, je puntík tmavější.]) představují mimořádně odlehlá pozorování či extrémy (tvoří dohromady méně než 1 % dat).
Samotné hodnoty jsou na ose "x" (horizontální osa) a tato osa má vždy rozsah podle toho, kolik bodů je v dané škále možné získat.

Samotné hodnoty jsou na ose "x" (horizontální osa) a tato osa má vždy rozsah podle toho, kolik bodů je v dané škále možné získat. První z krabicových grafů k dané položce vždy hovoří o výsledcích Vaší školy, druhý o školách ostatních.

<!-- ![Popis krabicového grafu ($\sigma$ značí směrodatnou odchylku, spodní část obrázku znázorňuje rozložení hodnot)](figures/boxplot_description.pdf) -->

```{=latex}
\begin{figure}
{\centering \includegraphics[width=0.75\textwidth]{figures/boxplot_description.pdf}}
\caption{Popis krabicového grafu ($\sigma$ značí směrodatnou odchylku, spodní část obrázku znázorňuje rozložení hodnot)}\label{fig:boxplotDesc}
\end{figure}
```

### Skládaný sloupcový graf

Skládaný sloupcový graf vizualizuje podíl jednotlivých odpovědí (klíč k barvám je v legendě) pro jednotlivé položky dotazníku. Čím větší plocha, tím větší má daná volba zastoupení. Osa "x" je tedy vyjádřená v procentech.

Na grafu č. \@ref(fig:stackedBarPlot) můžete vidět příklad, kde je u každé položky dvojice "řádků". První z nich, označený hvězdičkou, vždy referuje k Vaší škole, druhý ukazuje průměr všech *ostatních* škol, které se dotazníkového šetření účastnily. Jednotlivé položky řadíme ve zprávě vždy podle součtu dvou nejvíce pozitivních kategorií (v případě grafu č. \@ref(fig:stackedBarPlot) jde o "Určitě ano" a "Spíše ano"), jak se objevují v ostatních školách (čili v "průměru").  

```{r stackedBarPlot, fig.asp=.33, fig.cap="Ukázka skládaného sloupcového grafu"}
read_rds(here("data-input/barchart_sample_data.rds")) %>%
  ggplot(aes(
    y = digest,
    fill = value
  )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  scale_fill_schola(
    palette = "RdYlBu",
    direction = 1,
    na_pos = "first",
    labels = function(x) {
      str_wrap(x, width = 18)
    }
  ) +
  facet_wrap(
    ~ name %>% fct_reorder(value %>% str_detect("(Určitě ano)|(Spíše ano)"), mean, .desc = TRUE),
    dir = "v",
    ncol = 1
  ) +
  guides(
    fill = guide_legend(
      reverse = TRUE,
      nrow = 1,
      override.aes = list(size = NULL)
    ),
    alpha = guide_none()
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(2, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)),
    panel.spacing = unit(0, "mm"),
    strip.text = element_text(vjust = 0, margin = margin(l = 1.1 * 11 / 2, b = 1.5))
  )
```


<!-- \newpage -->

# Výsledky

Níže prezentujeme výsledky odpovědí na otázky pro všechny škály s rozsahem 1--4 body s tím, že žádoucí je vždy vyšší bodový zisk.

```{r scalesTotScores, include=FALSE}
scale_scores <- d %>%
  mutate(across(c(
    s1q1_a:s1q1_e,
    s1q3_a:s1q3_d,
    s1q4_a:s1q4_d,
    s2q1_a:s2q1_f,
    s2q2_a:s2q2_i,
    s4q3_a:s4q3_g,
    s6q1_a:s6q2_e,
    s6q4_a:s6q4_d,
    s7q1_a:s7q1_e
  ), ~ .x %>%
    drop_dontknow("Nevím / nechci odpovědět") %>% # destroy the NA without messing the values
    as.integer())) %>%
  mutate( # reverse-coded items
    s6q1_b = 5 - s6q1_b,
    s6q4_a = 5 - s6q4_a
  ) %>%
  rowwise() %>%
  mutate(
    tot_s1q1_T3STUD = mean(c_across(s1q1_a:s1q1_e), na.rm = TRUE),
    tot_s1q3_T3TEAM = mean(c_across(s1q3_a:s1q3_d), na.rm = TRUE),
    tot_s1q4_weak = mean(c_across(s1q4_a:s1q4_d), na.rm = TRUE),
    tot_s2q1_T3COOP = mean(c_across(s2q1_a:s2q1_f), na.rm = TRUE),
    tot_s2q1_T3EXCH = mean(c_across(c(s2q1_c:s2q1_e)), na.rm = TRUE),
    tot_s2q1_T3COLES = mean(c_across(c(s2q1_a:s2q1_b, s2q1_f)), na.rm = TRUE),
    tot_s2q2_T3SELF = mean(c_across(s2q2_a:s2q2_i), na.rm = TRUE),
    tot_s2q2_T3SEENG = mean(c_across(c(s2q2_a, s2q2_b, s2q2_d, s2q2_f)), na.rm = TRUE),
    tot_s2q2_T3SECLS = mean(c_across(c(s2q2_c, s2q2_e, s2q2_g, s2q2_h)), na.rm = TRUE),
    tot_s4q3_leader = mean(c_across(s4q3_a:s4q3_g), na.rm = TRUE),
    tot_s6q1_T3WELS = 5 - mean(c_across(s6q1_a:s6q1_d), na.rm = TRUE), # more points = better (unifying interpret. w/ others)
    tot_s6q2_T3WLOAD = 5 - mean(c_across(s6q2_a:s6q2_e), na.rm = TRUE), # more points = better
    tot_s6q4_T3JSENV = mean(c_across(s6q4_a:s6q4_d), na.rm = TRUE),
    tot_s7q1_teach_percept = mean(c_across(s7q1_a:s7q1_e), na.rm = TRUE),
  )

scales_labs_list <- list(
  "tot_s1q1_T3STUD" = "Vztahy učitel-žák a učitel-učitel",
  "tot_s1q3_T3TEAM" = "Inovativnost a angažovanost učitelského sboru",
  "tot_s1q4_weak" = "Dovednosti v práci s nejslabšími žáky",
  "tot_s2q1_T3COOP" = "Spolupráce mezi učiteli",
  "tot_s2q1_T3EXCH" = "Spolupráce v rámci vyučovacích hodin",
  "tot_s2q1_T3COLES" = "Výměna zkušeností, spolupráce v přípravě a řízení výuky",
  "tot_s2q2_T3SELF" = "Vnímaná osobní účinnost (self-efficacy)",
  "tot_s2q2_T3SEENG" = "Dovednost žáky zaujmout a zapojit*",
  "tot_s2q2_T3SECLS" = "Zvládání organizace hodiny a řízení jejího průběhu*",
  "tot_s4q3_leader" = "Spokojenost s vedením školy",
  "tot_s6q1_T3WELS" = "Pracovní spokojenost",
  "tot_s6q2_T3WLOAD" = "Pracovní vytížení a stres",
  "tot_s6q4_T3JSENV" = "Spokojenost s pracovním prostředím / školou",
  "tot_s7q1_teach_percept" = "Vnímaná hodnota povolání pedagoga a spokojenost"
)


# "tot_s2q1_T3COOP"
# "tot_s2q1_T3EXCH"
# "tot_s2q1_T3COLES"
# are 1:6, others are 1:4
```


```{r scales1to4, fig.asp=.9, fig.cap='Všechny škály s rozsahem 1--4 body'}
scale_scores %>% # select all total scores of the scales, except T3xxxx
  pivot_longer(c(
    starts_with("tot_"),
    -c(
      tot_s2q1_T3COOP,
      tot_s2q1_T3EXCH,
      tot_s2q1_T3COLES
    )
  )) %>%
  ggplot(aes(
    value,
    y = name %>% fct_reorder(value, median, na.rm = T),
    alpha = digest
  )) +
  # geom_point(col = "gray30", position = position_jitter(width = 1), size = 1, alpha = .1, show.legend = FALSE) +
  geom_boxplot(
    col = "gray25",
    fill = "gray90",
    size = .35,
    show.legend = FALSE,
    outlier.shape = "circle filled",
    outlier.alpha = .1,
    outlier.fill = "gray25",
    outlier.stroke = 0
  ) +
  scale_y_discrete(
    labels = function(x) {
      get_labs(x, 40, labs_list = scales_labs_list)
    }
  ) +
  scale_alpha_discrete(range = c(0, 1)) +
  labs(x = "body", caption = "*dílčí škály odvozené z „Vnímané osobní účinnosti (self-efficacy)“") +
  theme_schola("x") +
  theme(
    panel.grid.major.y = element_line(),
    axis.title.x = element_text(),
    plot.caption = element_text(face = "italic")
  )
```

```{r scales1to6, fig.asp=.4, fig.cap='Spolupráce mezi učiteli (škály s rozsahem 1--6 bodů)'}
scale_scores %>%
  pivot_longer(c(
    tot_s2q1_T3COOP,
    tot_s2q1_T3EXCH,
    tot_s2q1_T3COLES
  )) %>%
  ggplot(aes(value,
    y = name %>% fct_reorder(value, median, na.rm = TRUE),
    alpha = digest
  )) +
  geom_boxplot(
    col = "gray25", fill = "gray90", size = .35, show.legend = FALSE,
    outlier.shape = "circle filled", outlier.alpha = .1,
    outlier.fill = "gray25", outlier.stroke = 0
  ) +
  # geom_point(col = "gray30", position = position_jitter(), size = 1, alpha = .2, show.legend = FALSE) +
  scale_x_continuous(limits = c(1, 6), n.breaks = 6) +
  scale_y_discrete(labels = function(x) {
    get_labs(x, 40, labs_list = scales_labs_list)
  }) +
  scale_alpha_discrete(range = c(0, 1)) +
  labs(
    x = "body",
    caption = "Pozn.: Souhrnná škála „Spolupráce mezi učiteli“ je průměrem zbylých dvou."
  ) +
  theme_schola("x") +
  theme(
    panel.grid.major.y = element_line(), axis.title.x = element_text(),
    plot.caption = element_text(face = "italic")
  )
```


Následující grafy ukazují položky, které nenáleží do žádné škály, a tak je prezentujeme samostatně. Dělení do skupin vychází čistě z možností odpovědí (otázky mají stejnou legendu). Pro lepší orientaci řadíme položky podle součtu dvou nejvíce kladných možností.

```{r include=FALSE}
# items not included in scales
separate_items_names <- d %>%
  select(
    s1q2_a:s1q2_c,
    s2q3_a,
    s2q4_b,
    s2q3_b,
    s2q3_c,
    s2q4_a,
    s2q4_c,
    s3q1_a:s3q2,
    s4q1_a:s4q1_c,
    s5q1_a:s5q1_c,
    s6q3_a:s6q3_c,
    s0q1,
    s0q3,
    s0q4_a, s0q4_b
  ) %>%
  names()

# separate_items_names %>% get_labs() %>% view

separate_items <- items %>% filter(item %in% separate_items_names)

separate_items %>% count(levels)
```


```{r likert, fig.asp=1.6, fig.cap="Položky vyjadřující míru souhlasu pomocí 4 kategorií"}
likert_items <- separate_items %>%
  filter(resp_type == "agree") %>%
  pull(item)

d %>%
  select(all_of(likert_items), digest) %>%
  pivot_longer(-digest) %>%
  ggplot(aes(y = digest, fill = value, )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  scale_fill_schola(
    palette = "RdYlBu",
    direction = 1,
    na_pos = "first",
    labels = function(x) {
      str_wrap(x, width = 15)
    }
  ) +
  facet_wrap(
    ~ name %>% fct_reorder(value %>% str_detect("(?<!e)[S|s]ouhlasím"), mean, .desc = TRUE),
    dir = "v",
    ncol = 1,
    labeller = schola_labeller(wrap = 115)
  ) +
  guides(
    fill = guide_legend(
      reverse = T,
      nrow = 1,
      override.aes = list(size = NULL)
    ),
    alpha = guide_none()
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(1, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)),
    panel.spacing = unit(0, "mm"),
    strip.text = element_text(
      vjust = 0,
      margin = margin(l = 1.1 * 11 / 2, b = 1.5)
    )
  )
```




```{r midpoint, fig.asp=.375, fig.cap="Položky vyjadřující míru souhlasu pomocí 5 kategorií"}
midpoint_items <- separate_items %>%
  filter(resp_type == "midpoint") %>%
  pull(item)

d %>%
  pivot_longer(all_of(midpoint_items)) %>%
  ggplot(aes(
    y = digest,
    fill = value,
  )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  scale_fill_schola(
    palette = "RdYlBu",
    direction = 1,
    na_pos = "first",
    labels = function(x) {
      str_wrap(x, width = 18)
    }
  ) +
  facet_wrap(
    ~ name %>% fct_reorder(value %>% str_detect("(Určitě ano)|(Spíše ano)"), mean, .desc = TRUE),
    dir = "v",
    ncol = 1,
    labeller = schola_labeller(wrap = 115)
  ) +
  guides(
    fill = guide_legend(
      reverse = TRUE,
      nrow = 1,
      override.aes = list(size = NULL)
    ),
    alpha = guide_none()
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(2, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)), panel.spacing = unit(0, "mm"),
    strip.text = element_text(vjust = 0, margin = margin(l = 1.1 * 11 / 2, b = 1.5))
  )
```



```{r mentoring, fig.asp=.45, fig.cap="Formální i neformální mentorství"}
dont_know_dont_want_to_tell_items <- separate_items %>%
  filter(str_detect(levels, "Nevím / nechci odpovědět.*Ano.*Ne")) %>%
  pull(item)

d %>%
  pivot_longer(all_of(dont_know_dont_want_to_tell_items)) %>%
  ggplot(aes(
    y = digest,
    fill = value %>% fct_rev() %>% fct_relevel("Ne", "Ano", after = 1),
  )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  scale_fill_manual(
    values = c("darkgray", "#CA0020", "#0571B0"),
    drop = FALSE
  ) +
  facet_wrap(
    ~ name %>% fct_reorder(value == "Ano", mean, .desc = TRUE),
    dir = "v",
    ncol = 1,
    labeller = schola_labeller(wrap = 115)
  ) +
  guides(
    fill = guide_legend(
      reverse = TRUE,
      nrow = 1,
      override.aes = list(size = NULL)
    ),
    alpha = guide_none()
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(2, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)),
    panel.spacing = unit(0, "mm"),
    strip.text = element_text(vjust = 0, margin = margin(l = 1.1 * 11 / 2, b = 1.5))
  )
```


```{r profDevelOutliers}
d <- d %>% mutate(s3q5_outliers = if_else(scale(s3q5) > qnorm(1 - (.01 / 2)), TRUE, FALSE)) # 0.1 pval

s3q5_outliers_vals_curr_school <- d %>%
  filter(s3q5_outliers & digest) %>%
  arrange(s3q5) %>%
  pull(s3q5)

s3q5_outliers_vals_others <- d %>%
  filter(s3q5_outliers & !digest) %>%
  arrange(s3q5) %>%
  pull(s3q5)

s3q5_any_outliers <- (length(s3q5_outliers_vals_curr_school) + length(s3q5_outliers_vals_others)) != 0
```


```{r profDevelop, fig.cap=tidy_labs(s3q5), fig.asp=.19}
d %>%
  filter(!s3q5_outliers) %>%
  ggplot(aes(s3q5, digest, alpha = digest)) +
  geom_boxplot(
    col = "gray25", fill = "gray90", size = .35, show.legend = FALSE,
    outlier.shape = "circle filled", outlier.alpha = .1,
    outlier.fill = "gray25", outlier.stroke = 0
  ) +
  scale_alpha_discrete(range = c(0, 1)) +
  labs(x = "počet hodin") +
  theme_schola("x", axis.text.y = element_blank(), axis.title.x = element_text())
```


```{r profDevelText, results='asis', include = s3q5_any_outliers}
no_of_s1q5_outliers <- length(c(s3q5_outliers_vals_curr_school, s3q5_outliers_vals_others))

cat(paste0(
  "```{=latex}\n", # prevent pandoc filters from inserting NBSP
  "Z grafu výše jsme odstranili celkem ", no_of_s1q5_outliers, " extrémně ",
  case_when(
    no_of_s1q5_outliers == 1 ~ "odlehlou hodnotu",
    no_of_s1q5_outliers %in% 2:4 ~ "odlehlé hodnoty",
    TRUE ~ "odlehlých hodnot"
  ),
  ", aby byl lépe čitelný. ",

  ifelse(length(s3q5_outliers_vals_curr_school) != 0, paste0("Odlehlé hodnoty z Vaší školy: ", s3q5_outliers_vals_curr_school, ". "), "Z Vaší školy nebyly žádné takové hodnoty. "),

  ifelse(length(s3q5_outliers_vals_others) != 0, paste0("Odlehlé hodnoty z ostatních škol: ", paste0(s3q5_outliers_vals_others, collapse = ", "), "."), "Z ostatních škol nebyly žádné takové hodnoty."),
  "\n```"
))
```


\newpage

```{r whatToPlot}
plot_comparison <- d %>%
  filter(digest) %>%
  pull(s3q3) %>%
  magrittr::is_in("Ano") %>%
  any()
```


```{r, fig.cap="Vnímání dopadu zpětné vazby", fig.asp=.25}
d %>%
  pivot_longer(s3q3) %>%
  filter(!is.na(value)) %>%
  ggplot(aes(
    y = digest,
    fill = value %>% fct_rev() %>% fct_relevel("Ne", "Ano", after = 1)
  )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_fill_manual(values = c("darkgray", "#CA0020", "#0571B0"), drop = FALSE) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  facet_wrap(
    ~ name %>% fct_reorder(value == "Ano", mean, .desc = TRUE),
    dir = "v",
    ncol = 1,
    labeller = schola_labeller(wrap = 115)
  ) +
  guides(
    fill = guide_legend(
      reverse = TRUE,
      nrow = 1,
      override.aes = list(size = NULL)
    ),
    alpha = guide_none()
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(2, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)),
    panel.spacing = unit(0, "mm"),
    strip.text = element_text(vjust = 0, margin = margin(l = 1.1 * 11 / 2, b = 1.5))
  )
```


```{r feedbackSingle, include = !plot_comparison, fig.cap="Vnímání pozitivního dopadu zpětné vazby na jednotlivé aspekty výuky (pouze za ostatní školy)", fig.asp = .4}
d %>%
  pivot_longer(s3q4_a:s3q4_f) %>%
  filter(!is.na(value)) %>%
  ggplot(aes(
    y = name %>% fct_reorder(value == "Ano", mean, .desc = TRUE),
    fill = value %>% fct_rev() %>% fct_relevel("Ne", "Ano", after = 1)
  )) +
  geom_bar(position = "fill", col = "white", size = 1.1) +
  scale_y_discrete(labels = function(x) get_labs(x, 40)) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_fill_manual(values = c("darkgray", "#CA0020", "#0571B0"), drop = FALSE) +
  guides(fill = guide_legend(reverse = T, nrow = 1, override.aes = list(size = NULL))) +
  theme_schola("x",
    legend.title = element_blank(),
    legend.position = "top"
  )
```


```{r feedbackComp, include=plot_comparison, fig.cap="Vnímání pozitivního dopadu zpětné vazby na jednotlivé aspekty výuky", fig.asp = .65}
d %>%
  pivot_longer(s3q4_a:s3q4_f) %>%
  filter(!is.na(value)) %>%
  ggplot(aes(
    y = digest,
    fill = value %>% fct_rev() %>% fct_relevel("Ne", "Ano", after = 1),
  )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_fill_manual(values = c("darkgray", "#CA0020", "#0571B0"), drop = FALSE) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  facet_wrap(
    ~ name %>% fct_reorder(value == "Ano", mean, .desc = TRUE),
    dir = "v",
    ncol = 1,
    labeller = schola_labeller(wrap = 115)
  ) +
  guides(
    fill = guide_legend(
      reverse = TRUE,
      nrow = 1,
      override.aes = list(size = NULL)
    ),
    alpha = guide_none()
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(2, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)),
    panel.spacing = unit(0, "mm"),
    strip.text = element_text(vjust = 0, margin = margin(l = 1.1 * 11 / 2, b = 1.5))
  )
```





# Základní údaje o respondentech

Tato sekce shrnuje základní, zejména sociodemografické údaje o učitelích, kteří dotazník vyplnili. Dále zachycuje jejich pedagogické zkušenosti, délku učitelského úvazku a zprostředkovaně také motivaci k volbě povolání (viz graf č. \@ref(fig:career)). Graf č. \@ref(fig:subjects) odkazuje na skladbu vyučovaných předmětů, resp. jejich kategorií.

## Vaše škola

Ředitel/ka odeslal/a pozvánku k vyplnění dotazníku celkem `r total_invited` učitelkám a učitelům.
Dotazník jich otevřelo `r opened_count` a `r completed` u nich ho vyplněný odeslalo. Návratnost z pozvaných je tedy `r response_rate_invited`, návratnost efektivní pak činí `r response_rate_effective`.

```{r, results='asis', include=any_outliers}
glue::glue("Ve Vaší škole se vyskytli respontenti s neobvyklým počtem chybějících ({n_miss_outl}) nebo \"únikových\"^[Odpovědi typu \"Nevím / Nechci uvést.\".] odpovědí ({n_esc_outl}).")
```

## Ostatní školy

Dotazníkového šetření se zúčastnilo dalších `r resp_stats %>% filter(!digest) %>% pull(n_total_sum)` učitelek a učitelů z `r d %>% filter(!digest) %>% pull(red_izo) %>% n_distinct` škol. Vyplněný dotazník odeslalo `r resp_stats %>% filter(!digest) %>% pull(n_submitted_sum)` pedagogů. Efektivní návratnost tedy u ostatních škol činí `r ((resp_stats %>% filter(!digest) %>% pull(n_submitted_sum))/(resp_stats %>% filter(!digest) %>% pull(n_total_sum))) %>% scales::percent(1, suffix = "\u00A0%")`.



```{r sex, fig.asp=.235, fig.cap="Zastoupení pohlaví"}
# only one item
d %>%
  ggplot(aes(
    y = digest,
    fill = s0q1 %>% fct_rev()
  )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_fill_manual(values = scales::hue_pal(direction = -1)(2), drop = FALSE) +
  scale_x_percent_cz() +
  scale_y_discrete(labels = c("", "*")) +
  guides(
    fill = guide_legend(reverse = TRUE, override.aes = list(size = NULL)),
    alpha = guide_none()
  ) +
  theme_schola("x",
    legend.position = "top",
    legend.title = element_blank(),
    axis.text.y = element_blank()
  )
```


```{r age, fig.cap = "Věk", fig.asp=.185}
d %>%
  ggplot(aes(s0q2, digest, alpha = digest)) +
  geom_boxplot(
    col = "gray25", fill = "gray90", size = .35, show.legend = FALSE,
    outlier.shape = "circle filled", outlier.alpha = .1,
    outlier.fill = "gray25", outlier.stroke = 0
  ) +
  # geom_point(col = "gray30", position = position_jitter(), size = 1, alpha = .2, show.legend = FALSE) +
  scale_alpha_discrete(range = c(0, 1)) +
  labs(x = "počet let") +
  theme_schola("x", axis.text.y = element_blank(), axis.title.x = element_text())
```


```{r career, fig.asp = .235, fig.cap="Učitelská profese jako první kariérní volba"}
d %>%
  pivot_longer(s0q3) %>%
  ggplot(aes(
    y = digest,
    fill = value %>% fct_relevel("Ne", "Ano", after = 1)
  )) +
  geom_bar(position = "fill", col = "white", size = 1.1) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  scale_fill_manual(values = c("darkgray", "#CA0020", "#0571B0"), drop = FALSE) +
  guides(
    fill = guide_legend(reverse = T, nrow = 1, override.aes = list(size = NULL)),
    alpha = guide_none()
  ) +
  theme_schola("x",
    legend.position = "top",
    legend.title = element_blank(),
    axis.text.y = element_blank()
  )
```

```{r job, fig.asp=.3, fig.cap="Současné pracovní úvazky na pozici učitele"}
d %>%
  pivot_longer(c(s0q4_a, s0q4_b)) %>%
  ggplot(aes(
    y = digest,
    fill = value %>% fct_rev() %>% fct_relabel(~ str_extract(.x, "(?<=\\().*%"))
  )) + # percentage inside brackets
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  scale_fill_schola(
    palette = "Blues", direction = 1, na_pos = "none",
    labels = function(x) str_wrap(x, width = 15)
  ) +
  guides(
    fill = guide_legend(
      title = "podíl plného úvazku", reverse = T, nrow = 1,
      override.aes = list(size = NULL)
    ),
    alpha = guide_none()
  ) +
  facet_wrap(
    ~ name %>%
      fct_reorder(value %>% str_detect("(než 90)|(-90)"), mean, .desc = TRUE),
    dir = "v",
    ncol = 1,
    labeller = schola_labeller(wrap = 115)
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(2, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)),
    panel.spacing = unit(0, "mm"),
    strip.text = element_text(vjust = 0, margin = margin(l = 1.1 * 11 / 2, b = 1.5))
  )
```


```{r yearsTeach, fig.asp=.3, fig.cap="Pracovní zkušenosti"}
d %>%
  pivot_longer(c(s0q5_a, s0q5_b)) %>%
  ggplot(aes(value,
    y = name %>% fct_reorder(value, median, na.rm = TRUE),
    alpha = digest
  )) +
  geom_boxplot(
    col = "gray25", fill = "gray90", size = .35, show.legend = FALSE,
    outlier.shape = "circle filled", outlier.alpha = .1,
    outlier.fill = "gray25", outlier.stroke = 0
  ) +
  scale_x_continuous(n.breaks = 6) +
  scale_y_discrete(labels = function(x) {
    get_labs(x, 40, labs_list = item_labs_list)
  }) +
  scale_alpha_discrete(range = c(0, 1)) +
  labs(
    x = "počet let na pedagogické pozici"
  ) +
  theme_schola("x") +
  theme(
    panel.grid.major.y = element_line(), axis.title.x = element_text(),
    plot.caption = element_text(face = "italic")
  )
```



```{r s8q2, fig.asp =.18, fig.cap="Podíl výuky na druhém stupni"}
d %>%
  ggplot(aes(s8q2_b / 100, digest, alpha = digest)) +
  geom_boxplot(
    col = "gray25", fill = "gray90", size = .35, show.legend = FALSE,
    outlier.shape = "circle filled", outlier.alpha = .1,
    outlier.fill = "gray25", outlier.stroke = 0
  ) +
  # geom_point(col = "gray30", position = position_jitter(), size = 1, alpha = .2, show.legend = FALSE) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_alpha_discrete(range = c(0, 1)) +
  theme_schola("x", axis.text.y = element_blank())
```

Z grafu výše je patrné, že většina pedagogů vyučuje buď 100% na prvním, nebo nebo 100% na druhém stupni. Po očištění dat od těchto případů vypadá situace takto:

```{r s8q2Subset, fig.asp =.18, fig.cap="Podíl výuky na druhém stupni bez pedagogů vyučujících pouze na jednom stupni"}
d %>%
  filter(s8q2_b > 0 & s8q2_b < 100) %>%
  ggplot(aes(s8q2_b / 100, digest, alpha = digest)) +
  geom_boxplot(
    col = "gray25", fill = "gray90", size = .35, show.legend = FALSE,
    outlier.shape = "circle filled", outlier.alpha = .1,
    outlier.fill = "gray25", outlier.stroke = 0
  ) +
  # geom_point(col = "gray30", position = position_jitter(), size = 1, alpha = .2, show.legend = FALSE) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_alpha_discrete(range = c(0, 1)) +
  theme_schola("x", axis.text.y = element_blank())
```


```{r subjects, fig.cap = "Počty pedagogů vyučujících jednotlivé kategorie předmětů", fig.asp = .99}

d %>%
  pivot_longer(s8q1_a:s8q1_k) %>%
  # filter(!is.na(value)) %>%
  ggplot(aes(
    y = digest,
    fill = value %>% fct_rev() %>% fct_relevel("Nevybráno", "Ano", after = 1)
  )) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
    width = 1
  ) +
  # scale_y_discrete(labels = function(x) get_labs(x, 40)) +
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_y_discrete(labels = c("", "*")) +
  scale_fill_manual(values = c("darkgray", "#0571B0"), drop = FALSE) +
  guides(
    fill = guide_legend(reverse = T, nrow = 1, override.aes = list(size = NULL)),
    alpha = guide_none()
  ) +
  facet_wrap(~ name %>% fct_reorder(value == "Ano", mean, .desc = TRUE),
    dir = "v",
    ncol = 1,
    labeller = schola_labeller(wrap = 115)
  ) +
  theme_schola(
    "x",
    legend.title = element_blank(),
    legend.position = "top",
    legend.box.spacing = unit(2, "mm"),
    axis.text.y = element_text(size = 12, face = "bold", vjust = .615, margin = margin(0, 0, 0, 0)),
    panel.spacing = unit(0, "mm"),
    strip.text = element_text(vjust = 0, margin = margin(l = 1.1 * 11 / 2, b = 1.5))
  )
```

Učitelé mají na starosti výuku různých předmětů, které vyobrazujeme v 11 kategoriích, jak je běžné v mezinárodních šetření typu PISA či TALIS. Modrá výplň ukazuje počet učitelů, kteří předmět či kategorii vyučují nebo vyučovali v tomto nebo předchozím školním roce. Platí, že jeden učitel může vyučovat více předmětů.

\newpage

# Příloha	

## Vizualizace položek dotazníku tvořících škály

Škály, které jste viděli na začátku zprávy, sestávají vždy z několika položek, na které se odpovídá stejným způsobem (např. míra souhlasu). V následující příloze si můžete podrobně prohlédnout, z jakých konkrétních otázek daná škála vychází a jak na ní učitelky a učitelé ve Vaší škole odpovídali.

```{r}
untidy_items <- function(..., .data = d) {
  expr <- rlang::expr(c(...))
  pos <- tidyselect::eval_select(expr, data = .data)
  pos %>% names()
}

scales_items <- list(
  tot_s1q1_T3STUD = untidy_items(s1q1_a:s1q1_e),
  tot_s1q3_T3TEAM = untidy_items(s1q3_a:s1q3_d),
  tot_s1q4_weak = untidy_items(s1q4_a:s1q4_d),
  tot_s2q1_T3COOP = untidy_items(s2q1_a:s2q1_f),
  tot_s2q1_T3EXCH = untidy_items(c(s2q1_c:s2q1_e)),
  tot_s2q1_T3COLES = untidy_items(c(s2q1_a:s2q1_b, s2q1_f)),
  tot_s2q2_T3SELF = untidy_items(s2q2_a:s2q2_i),
  tot_s2q2_T3SEENG = untidy_items(c(s2q2_a, s2q2_b, s2q2_d, s2q2_f)),
  tot_s2q2_T3SECLS = untidy_items(c(s2q2_c, s2q2_e, s2q2_g, s2q2_h)),
  tot_s4q3_leader = untidy_items(s4q3_a:s4q3_g),
  tot_s6q1_T3WELS = untidy_items(s6q1_a:s6q1_d),
  tot_s6q2_T3WLOAD = untidy_items(s6q2_a:s6q2_e),
  tot_s6q4_T3JSENV = untidy_items(s6q4_a:s6q4_d),
  tot_s7q1_teach_percept = untidy_items(s7q1_a:s7q1_e)
)

scales_items %<>% tibble(scale = names(.), item = .) %>%
  unnest(cols = c(item)) %>%
  nest_by(item) %>%
  transmute(scale = flatten_chr(data)[1], subscale = flatten_chr(data)[2])

# append scale and subscale membership to the main item overview
items %<>% left_join(scales_items, by = "item")


# only scales items
appdx_items <-
  items %>%
  filter(!is.na(scale)) %>%
  select(item, scale, resp_type) %>%
  group_by(scale) %>%
  summarise(items = list(cur_data()$item), resp_type = cur_data()$resp_type[1]) %>%
  ungroup() %>%
  mutate(
    fig_asp = case_when(
      scale == "tot_s1q1_T3STUD" ~ .55,
      scale == "tot_s1q3_T3TEAM" ~ .48,
      scale == "tot_s2q1_T3COOP" ~ .7,
      scale == "tot_s1q4_weak" ~ .48,
      scale == "tot_s2q2_T3SELF" ~ .89,
      scale == "tot_s4q3_leader" ~ .73,
      scale == "tot_s6q1_T3WELS" ~ .48,
      scale == "tot_s6q2_T3WLOAD" ~ .57,
      scale == "tot_s6q4_T3JSENV" ~ .49,
      scale == "tot_s7q1_teach_percept" ~ .6
    ),
    regex = case_when(
      resp_type == "agree" ~ "(?<!e)[S|s]ouhlasím",
      resp_type == "considerable_extent" ~ "(Do značné)|(Do určité)",
      resp_type == "large_extent" ~ "(Do velké)|(Docela ano)",
      resp_type == "time" ~ "(týden)|(1-3)|(5-10)"
    ),
    palette = if_else(resp_type == "time", "Blues", "RdYlBu"),
    legend_nrow = if_else(resp_type == "time", 2, 1)
  )
```



```{r, echo=FALSE, results='asis'}
src_appdx <- pmap(
  list(
    appdx_items$scale, # ..1
    appdx_items$items, # ..2 (another variable uses ..3 and so on)
    appdx_items$fig_asp,
    appdx_items$regex,
    appdx_items$palette,
    appdx_items$legend_nrow
  ),
  ~ knit_expand(
    file =
      here("templates", "appdx_scales.Rmd"),
    items_of_scale = ..2 %>% str_flatten(collapse = ", "),
    scale = ..1,
    chunk = ..1 %>% str_remove_all("_") %>% str_c("appdx"), # chunk names MUSTN'T contain any underscores!
    regex = ..4,
    fig_asp = ..3,
    palette = ..5,
    legend_nrow = ..6
  ) # you can add any number of additional template variables
) %>% set_names(appdx_items$scale) # pluck by item code for convenience

# note:
# it is absolutely critical to insert double linebreak, as MD iselft doesn't
# create any linebreaks that are crucial in case of Word rendering
# w/o those lnbrks, Word renders just the plots with no captions whatsoever...

# add linebreak at the end if it is missing
src_appdx %<>% map(~ if (str_detect(.x, "\\n$")) {
  .x
} else {
  paste0(.x, "\n")
})

# render
# opts_knit$set(fig_path = "figures")
knitr::knit(text = unlist(src_appdx), quiet = TRUE) %>% cat()
```
