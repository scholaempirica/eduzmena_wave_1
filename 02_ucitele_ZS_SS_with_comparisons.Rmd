...
logo: logos/schola # for path with _ use "`logos/schola_logo.pdf`{=latex}"
logo_height: 15pt
...
# add selectionvariable indicating selected school
d %<>% mutate(digest = if_else(red_izo == "1", TRUE, FALSE))
...
curr_school <- d %>% filter(red_izo == params$redizo)

total_teachers <- nrow(curr_school)
...
response_rate <- percent(completed/total_invited, 1, suffix = "\u00A0%", decimal.mark = ",")
...
scale_scores %>%
  ...
  geom_boxplot(
    col = "gray25", fill = "gray90", size = .25, show.legend = FALSE,
...
d %>%
  ...
  pivot_longer(-digest) %>%
  ggplot(aes(y = digest, fill = value, alpha = digest)) +
  geom_bar(
    col = "white",
    size = .7,
    position = "fill",
...
d %>%
  ...
  scale_x_percent_cz(limits = c(0, 1)) +
  scale_fill_manual(values = c("darkgray", "#CA0020", "#0571B0"),
                    drop = FALSE) +
  scale_alpha_discrete(range = c(.5, 1)) +
  facet_wrap(
