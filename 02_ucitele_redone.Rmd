...
- a Marek Havrda
date: '`r gsub("^0", "", format(Sys.time(), "%d. %B %Y"))`' # remove leading zero
params:
  redizo: "600099296" # there must be a value, to smoothly compile oustide Build script
  pilot: true
  fig_asp_body: .6
  fig_asp_appdx: .4
  alpha_lower: .6
...
# those two functions cannot be sources as a part of shared.R for UNKNOWN REASON!!
# bitewise, everything is correct, but knitting within the rmarkdown session messes up the encoding somehow...
...
# if pilot
if (params$pilot) {
  d <- read_rds(here("data-processed", "ucitele_semilsko.rds")) # real semilsko pilot data
  d %<>% filter(red_izo == params$redizo) # only one in pilot
  alpha_lower <- 1 # if only one group, make it solid
} else {
  d <- read_rds(here("data-processed", "ucitele_simulated.rds"))
}
...
item_labs_list <- read_rds(here("data-processed/ucitele_semilsko_labels.rds"))
...
if (!file.exists(here("data-processed", "item_labs_edit.csv"))) {
  item_labs_list %>%
    unlist() %>%
    enframe(name = "code", value = "wording") %>%
    write_excel_csv2(here("data-processed", "item_labs_edit.csv"))
}

# import edited labels
item_labs_edited <- read_delim(here("data-processed/item_labs_edit.csv"),
...
![Obr. 1. Popis krabicového grafu ($\sigma$ značí směrodatnou odchylku, spodní část obrázku znázorňuje rozložení hodnot)](figures/boxplot_description.png)
...
```{r scales1to4, fig.asp=.9, fig.cap='Všechny škály s rozsahem 1-4 body^[Hvězdička značí, že se jedná o dílčí škály odvozené z "Vnímané osobní účinnosti (self-efficacy)"]'}
scale_scores %>%
  select(-c(
    tot_s2q1_T3COOP,
    tot_s2q1_T3EXCH,
...
```{r scales1to6, fig.asp=.3, fig.cap='Spolupráce mezi učiteli (škály s rozsahem 1-6 bodů)^[Souhrnná škála "Spolupráce mezi učiteli" je průměrem zbylých dvou.]'}
scale_scores %>%
  select(c(
    tot_s2q1_T3COOP,
    tot_s2q1_T3EXCH,
  ...
  theme_schola("x") + theme(panel.grid.major.y =  element_line())
```
...
Ředitelé odeslali pozvánku k vyplnění dotazníku celkem `r total_invited` učitelům a učitelkám. Z toho `r opened_count` dotazník otevřelo a `r completed` ho vyplněný odeslalo. Efektivní návratnost je tedy `r response_rate`.
